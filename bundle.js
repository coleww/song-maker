...
function doit(e){if(sq)sq.stop(),the_ui.remove();else{document.getElementById("notes").remove();var t=document.createElement("div");t.innerHTML='<img src="kitty.gif" style="width: 100%; height: 100%;">',document.body.appendChild(t)}var n=JSON.parse(JSON.stringify(the_song)),r=e.clipboardData.getData("text/plain");try{var s=proc.processTab(r,8);if(console.log(s),s.length){var i=Math.ceil(s.length/3);Object.keys(n.instruments).forEach(function(e){if(n.instruments[e].melodic){n.instruments[e].patterns.verse.notes=pick(s,{count:i});var t=n.instruments[e].patterns.verse.notes.length;n.instruments[e].patterns.verse.nexts=n.instruments[e].patterns.verse.notes.map(function(e,n){return[~~(Math.random*t),n,~~(Math.random*t)]}),n.instruments[e].patterns.verse.probs=n.instruments[e].patterns.verse.notes.map(function(e){return e.map(function(e){return e.length?.25*Math.random()+.5:0})})}n.instruments[e].play=function(t){var r=n.instruments[e].config||{},s=n.instruments[e].multi||1,i=n.instruments[e].melodic?{freq:int2freq(t,n.key)*s}:{};synths[e].update(merge(i,r),ac.currentTime),synths[e].start(ac.currentTime)}}),the_ui=document.createElement("div"),the_ui.id="main",the_ui.className="super-simple layover",sq=seq(n),console.log(sq);var a=document.createElement("h4");a.textContent="ROCK ON, KEYBOARD CAT!",the_ui.appendChild(a);var o=document.createElement("scale-select");the_ui.appendChild(o),o.addEventListener("tonic",function(e){var t=e.detail.value;n.key.tonic=t+3,sq.updateSong(n)}),o.addEventListener("scale",function(e){var t=e.detail.value;n.key.scale=t,console.log(sq),sq.updateSong(n)});var c=document.createElement("openmusic-slider");c.min=100,c.max=300,console.log(c),c.value=320,the_ui.appendChild(c),c.addEventListener("change",function(e){console.log(e),console.log("whoa"),n.bpm=2*~~e.target.value,sq.stop(),sq.updateSong(n),sq.start()}),document.body.appendChild(the_ui),sq.start()}else console.log("NOTHING THERE?")}catch(e){console.log(e),alert("very sorry, something broke, not to worry, refresh the page and perhaps another guitar or bass tab will work better?")}}var proc=require("./processtab");window.AudioContext=window.AudioContext||window.webkitAudioContext;var ac=new AudioContext,pie=require("pie-ano"),mainVolume=ac.createGain();mainVolume.connect(ac.destination),mainVolume.gain.setValueAtTime(.5,ac.currentTime);var synths={snare:require("dj-snazzy-snare")(ac),kick:require("touch-down-dance")(ac),hat:require("really-hi-hat")(ac),bb:pie(ac),pp:pie(ac),sm:pie(ac)};require("document-register-element"),require("scale-select").register("scale-select"),require("openmusic-slider").register("openmusic-slider"),Object.keys(synths).forEach(function(e){synths[e].connect(mainVolume)});var seq=require("spiderbite"),the_song=require("./demoSong"),pick=require("pick-random"),merge=require("merge"),int2freq=require("int2freq"),sq,the_ui;document.body.addEventListener("paste",doit),document.getElementById("theinput").addEventListener("paste",doit);
